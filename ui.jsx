...
// ---------- Primitives ----------
function Button({ variant = "default", href, external, onClick, children, icon, ...rest }) {
...
function CoordBox({ children }) {
  return React.createElement("span", { className: "cad-coord" }, children);
}
function TickRuler({ count = 7, w = 13, h = 78 }) {
  const ticks = [];
...
function DimLine({ label = "", w = 168, bg = "var(--bg)" }) {
  const h = 16;
...
function SectionHeader({ eyebrow, title, lead, coord, fig }) {
...
Object.assign(window, {
  GitHubIcon, ArrowUpRight, BookIcon, CheckIcon, MenuIcon, PlayIcon,
  Button, Tag, Badge, Card, useInView, Reveal, SectionHeader,
  CornerBrackets, Crosshair, CoordBox, TickRuler, DimLine, DiagGuide, SectionCAD,
});
